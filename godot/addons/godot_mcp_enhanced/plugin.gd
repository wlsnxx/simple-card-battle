...
func _enter_tree() -> void:
	...
	debugger_plugin = preload("res://addons/godot_mcp_enhanced/mcp_debugger_plugin.gd").new()
	add_debugger_plugin(debugger_plugin)
	screenshot_manager.debugger_plugin = debugger_plugin
	...
	runtime_operations = preload("res://addons/godot_mcp_enhanced/runtime_operations.gd").new()
	runtime_operations.name = "MCPRuntimeOperations"
	runtime_operations.editor_interface = get_editor_interface()
	runtime_operations.debugger_plugin = debugger_plugin
...
func _setup_http_routes() -> void:
	...
	http_server.register_route("/api/runtime/autoplay_start", _handle_autoplay_start)
	http_server.register_route("/api/runtime/autoplay_stop", _handle_autoplay_stop)
...
func _handle_get_running_scene_screenshot(params: Dictionary) -> Dictionary:
	var screenshot_data = await screenshot_manager.capture_running_scene_screenshot()
	return {"success": true, "data": {"screenshot": screenshot_data}}
...
func _handle_autoplay_start(params: Dictionary) -> Dictionary:
	var delay := float(params.get("reaction_delay", 0.3))
	return runtime_operations.autoplay_start(delay)


func _handle_autoplay_stop(_params: Dictionary) -> Dictionary:
	return runtime_operations.autoplay_stop()
