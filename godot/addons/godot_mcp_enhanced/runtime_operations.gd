...
func simulate_key_press(keycode: int, pressed: bool = true) -> Dictionary:
	...
	return {
		"success": true,
		"message": "Key %d %s (Editor only)" % [keycode, "pressed" if pressed else "released"]
	}
...
func simulate_action(action_name: String, pressed: bool = true, strength: float = 1.0) -> Dictionary:
	...
	if debugger_plugin and debugger_plugin.get_sessions().size() > 0:
		if action_name.begins_with("mouse:"):
			var parts = action_name.split(":")
			if parts.size() == 3:
				debugger_plugin.simulate_mouse(parts[1].to_float(), parts[2].to_float(), pressed)
				return {
					"success": true,
					"message": "Mouse click sent to runtime at " + parts[1] + "," + parts[2]
				}
		debugger_plugin.simulate_action(action_name, pressed)
		return {
			"success": true,
			"message": "Action '%s' %s sent to Runtime via Debugger" % [action_name, "pressed" if pressed else "released"]
		}
	...
	return {
		"success": true,
		"message": "Action '%s' %s with strength %.2f (Editor only)" % [action_name, "pressed" if pressed else "released", strength]
	}


func autoplay_start(reaction_delay: float = 0.3) -> Dictionary:
	if debugger_plugin == null:
		return {"success": false, "error": "debugger_plugin not set"}
	var sessions := debugger_plugin.get_sessions()
	if sessions.is_empty() or not sessions[0].is_active():
		return {"success": false, "error": "No active debug session — start the game first"}
	debugger_plugin.start_autoplay(reaction_delay)
	return {"success": true, "message": "AutoplayBot started (reaction_delay=%.2f)" % reaction_delay}


func autoplay_stop() -> Dictionary:
	if debugger_plugin == null:
		return {"success": false, "error": "debugger_plugin not set"}
	debugger_plugin.stop_autoplay()
	return {"success": true, "message": "AutoplayBot stop signal sent"}
